...
<div {{ $attributes->twMerge(['class' => 'py-4 rounded-xl border border-zinc-200 divide-y divide-zinc-200 divide-dashed']) }}>
    <dl class="px-4 pb-4 space-y-4">
        <div class="flex items-center justify-between text-sm">
            <dt class="text-zinc-700">{{ __('Subtotal') }} {{ current_tax_label() }}</dt>
        ...
        <div class="flex items-center justify-between text-sm">
            ...
            <dd class="font-medium text-zinc-900">
                {{ format_cents($order->tax_amount, $order->currency_code) }}
            </dd>
        </div>
        <div class="flex items-center justify-between pt-4 border-t border-zinc-200">
            ...
            <dd class="text-base font-bold text-zinc-900">
                {{ format_cents($order->price_amount, $order->currency_code) }}
            </dd>
        </div>
    </dl>
