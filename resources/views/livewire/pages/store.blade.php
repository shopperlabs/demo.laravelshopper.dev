<div class="bg-white">
    <div class="pb-16 sm:pb-24">
        <x-container class="max-w-2xl">
            <div class="mx-auto">
                <div class="border-b border-zinc-200 pb-4 pt-8 mb-2">
                    <h1 class="text-4xl font-bold tracking-tight text-zinc-900">{{ __('Store') }}</h1>
                                                ...
                                                <div class="space-y-3 pt-6">
                                                    @foreach ($attribute->values as $index => $value)
                                                        <div class="flex items-center" wire:key="{{ $attribute->slug }}-{{ $value->key }}">
                                                            ...
                                                            >
                                                            <label for="{{ $attribute->slug }}-{{ $index }}" class="ml-3 text-sm text-zinc-600">{{ $value->value }}</label>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            @endif
                            ...
                            <div class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 xl:gap-x-8">
                                @foreach($products as $product)
                                    <x-link :href="route('single-product', $product)" class="group">
                                        <x-products.thumbnail :product="$product" class="aspect-square w-full rounded-lg bg-zinc-200 object-cover group-hover:opacity-75 xl:aspect-[7/8]"/>
                                        <h3 class="mt-4 text-sm text-zinc-700">{{ $product->name }}</h3>
                                        <x-products.price :product="$product" class="mt-1 text-lg font-medium text-zinc-900"/>
                                    </x-link>
                                @endforeach
                            </div>
                            <div class="mt-2">
